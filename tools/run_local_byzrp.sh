...
for ((i=1; i<=N; i++)); do
    ...
    echo "running byz$i"
    docker run -d --rm -v $PWD/$IP.crt:/etc/ssl/certs/server.crt -v $PWD/$IP.rtr.crt:/etc/ssl/certs/rtr.crt -v $PWD/$IP.key:/etc/ssl/private/server.key -v $PWD/$IP.rtr.key:/etc/ssl/private/rtr.key -v $PWD/root.crt:/etc/ssl/certs/root.crt -v $PWD/peers.lst:/data/out/share/peers.lst -p 909$i:9090 -p 910$i:9100 --name byz$i $IMG #> /dev/null 2> /dev/null
